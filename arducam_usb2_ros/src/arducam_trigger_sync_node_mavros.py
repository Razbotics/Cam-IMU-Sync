...
def controlSignalCapture(enable):
    trigger_enable = enable
    reset_sequence = True
    trigger_pause = False

    while True:
      srv_response = capture_control_srv_(trigger_enable, reset_sequence, trigger_pause)
      if not srv_response.success:
        rospy.logwarn("Failed to call capture control service.")
        rospy.sleep(1.0) #sleep for one second each time
      elif srv_response.success:
          break

    if enable:
        rospy.loginfo("Enabled camera signal capture.")
    else:
        rospy.loginfo("Disabled camera signal capture.")
...
def time_subscriber_callback(time_data):
    ...
    if first_trigger:
      next_trigger_counter = time_data.frame_seq_id
      first_trigger = False

    if next_trigger_counter == time_data.frame_seq_id:
        ...
        if datasize > 0:
            ...
            try:
                ...
                exposure_time = rospy.Time.from_sec(cam_time_delay) #adding exposure time 20ms/2
                img_msg.header.stamp.secs = time_data.frame_stamp.secs + exposure_time.to_sec()
                img_msg.header.stamp.nsecs = time_data.frame_stamp.nsecs + exposure_time.to_nsec()
                img_msg.header.seq = time_data.frame_seq_id
        ...
        rospy.logwarn("trigger not in sync (seq expected %u, got %u)!", next_trigger_counter, time_data.frame_seq_id)
...
if __name__ == "__main__":
    ...
    time_subscriber = rospy.Subscriber("/mavros/cam_imu_sync/cam_imu_stamp", CamIMUStamp, time_subscriber_callback, queue_size=10)
    capture_control_srv_ = rospy.ServiceProxy("/mavros/cmd/trigger_control", CommandTriggerControl)
    bridge = CvBridge()
    ...
    grayscale = rospy.get_param("~grayscale", False)
    cam_time_delay = rospy.get_param("~cam_time_delay", 0.0)
    ...
    if camera_initFromFile(config_file_name):
        ...
        rospy.wait_for_service("/mavros/cmd/trigger_control")
        controlSignalCapture(True) #start trigger capture
